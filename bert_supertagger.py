...
def run(tsuite, supertags_path, grammar, ace_exec, output_path, gold_mrs, profile_name):
    ...
    start = time.time()
    results, n = run_ace_on_ts(tsuite, grammar, ace_exec, ['-1', '--max-chart-megabytes=24000', '--max-unpack-megabytes=32000', '-z', supertags_path], 'i-input', output_path + '/bert/' + profile_name)
